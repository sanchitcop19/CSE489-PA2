...
//std::cout("buffer_b: ");
    while (!q.empty()){
        //std::cout("seqnum: %i | ", q.front().seqnum);
        q.pop_front();
    }
    fflush(0);
}
void printqueue(std::deque <std::pair <struct pkt, float> > q){
    while (!q.empty()){
        //std::cout("seqnum: %i, expiration time: %0.5f | ", q.front().first.seqnum, q.front().second);
        q.pop_front();
    }
    fflush(0);
}
...
void A_input(struct pkt packet) {
    ...
    if (verify_checksum(packet) && packet.acknum >= send_base) {
        ...
        while (it != unacked.end()){
            ...
            //std::cout("seqnum: %i, min: %i\t", (*it).first.seqnum, min);
            it++;
        }
        //std::cout << std::endl;
    ...
    corrupted:
    //std::cout("received corrupted packet\n");
    //std::cout << "Status of unacked queue: [";
    printqueue(unacked);
    //std::cout << "]" << std::endl;
